...
VM_NAME=$(echo "$IMAGE_NAME" | sed 's/_/-/g; s/\.img$//')
...
# Remove the downloaded image after script finishes
trap "rm -f /tmp/$IMAGE_NAME" EXIT
...
# Check if qemu-guest-agent should be installed
if [ "$QEMU_AGENT" = true ]; then
    # Customize cloud init image
    echo "Customizing..."
    virt-customize --add $IMAGE_NAME --install qemu-guest-agent
else
    echo "QEMU_AGENT is not enabled. Skipping qemu-guest-agent installation."
fi
...
# VM Disc import
echo "Importing disk..."
qm disk import $VM_ID $TEMP_DIR/$IMAGE_NAME ${STORAGE:-local-lvm}

# VM Configuration
echo "Configuring VM..."
qm set $VM_ID --scsihw virtio-scsi-pci --scsi0 ${STORAGE:-local-lvm}:vm-$VM_ID-disk-0
qm set $VM_ID --ide2 ${STORAGE:-local-lvm}:cloudinit #Create cloud-init disk    
qm set $VM_ID --boot c --bootdisk scsi0 # Set boot disk
qm set $VM_ID --serial0 socket --vga serial0
qm set $VM_ID --agent enabled=1 # Enable QEMU agent

# Disk resize
echo "Disk resize..."
qm disk resize $VM_ID scsi0 ${DISK_SIZE:-"+50G"}
